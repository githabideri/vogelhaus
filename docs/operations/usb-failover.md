...
**On the host Pi (e.g., Pi 4 at `<PI4_USB_IP>`):**
```bash
PEER_USB_IP="<ZERO_USB_IP>"
CHECK_INTERVAL=30
```

**On the gadget Pi (e.g., Pi Zero at `<ZERO_USB_IP>`):**
```bash
PEER_USB_IP="<PI4_USB_IP>"
CHECK_INTERVAL=30
```
...
ssh-copy-id vb-light@<ZERO_USB_IP>    # via USB
ssh-copy-id vb-light@<ZERO_LAN_IP>    # via LAN

# Deploy keys (from Pi Zero):
ssh-copy-id vogl@<PI4_USB_IP>         # via USB
```
...
**Important routing note:** The failover route uses `src <usb-ip>` to ensure return packets come back through USB, not through the LAN. Without this, NAT de-translation on the peer routes replies via LAN instead of USB, causing 100% packet loss.

If the peer Pi runs Docker, you also need FORWARD rules allowing USB traffic through Docker's firewall (FORWARD policy is DROP with Docker). See the Docker firewall section below.

## Docker Firewall (Pi 4 specific)

If the peer Pi runs Docker, the FORWARD chain policy is set to DROP. You need explicit rules to allow USB traffic through:
...
| Network | Pi 4 IP | Pi Zero IP | Purpose |
|---------|---------|------------|---------|
| USB Gadget | `<PI4_USB_IP>` | `<ZERO_USB_IP>` | Failover link |
| WiFi | `<PI4_WLAN_IP>` | `<ZERO_WLAN_IP>` | Primary connectivity |
| Ethernet | `<PI4_LAN_IP>` | N/A | Pi 4 wired backup |

### Routing Behavior

**Normal operation:**
- Both Pis use WiFi for internet access
- USB link available but not used for routing
- Each Pi has independent internet connectivity

**Failover mode:**
- Failed Pi routes via USB to working Pi
- Working Pi enables NAT/masquerading for failed Pi
- Return traffic routed correctly via source IP
...
sudo systemctl status usb-failover

# View current configuration and state
sudo usb-failover.sh --status

# View logs
sudo journalctl -u usb-failover -f
...
| Status | Meaning |
|--------|---------|
| `NORMAL` | WiFi working, no failover needed |
| `FAILOVER` | Using USB peer for internet |
| `PEER_DOWN` | USB peer unreachable |
| `NO_INTERNET` | Neither WiFi nor USB peer works |
...
| `/etc/systemd/system/usb-failover.service` | Systemd service definition |